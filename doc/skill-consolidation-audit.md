...
综合结论：本轮规则、真实媒体恢复和隔离接收方复验都已有证据。v1.37 加入主角 P0 准入门禁，明确禁止局部腿部反射并要求逐帧来源、锚点和真实地图连续帧；v1.38 再补充场景专用完整高度侧墙、统一等比定位后按门洞裁切、禁止短段分别缩放与局部镜像端面的合同。它可以继续用于验证新游戏，但正式团队稳定版仍应等一个独立项目把主角、方向门和整房高保真全部签收后再定级。
