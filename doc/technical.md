...
`npm run build`、`npm test`、技能包的 `validate-world` 与 `verify-space` 已通过；真实浏览器已完成居所 → 走廊 → 服务点 → 选择保留 → 原路返回 → 刷新 → 回访。借出分支目前通过权威状态测试，仍需真实浏览器和新玩家试读。`comprehension unverified`：自动路线不能证明玩家已经理解世界观。试玩中的代码类型适配把冻结 StoryCartridge 的三项固定 stat 元组改为数组，使本切片可合法使用零数值 HUD；此变更需回写技能试用记录，不能默认为所有旧游戏已支持零数值。
