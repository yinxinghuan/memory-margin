...
import {initialHead,migrateHead,resolveAction,type Head,type Action,type Result} from './journey'
...
export class JourneyStore{
 ...
 async load(locale:Locale){const tx=this.db.transaction('heads','readwrite'),done=finished(tx),store=tx.objectStore('heads');let head=await req(store.get('active')) as Head|undefined;if(!head){head=initialHead(locale);store.put(head,'active')}else{const migrated=migrateHead(head);if(migrated!==head){head=migrated;store.put(head,'active')}}await done;return head}
 async action(request:Action):Promise<Result>{
  ...
  try{const receipts=tx.objectStore('actions'),digest=JSON.stringify(request),old=await req(receipts.get(request.id));if(old){if(old.digest!==digest)throw new Error('ACTION_ID_CONFLICT');await done;return old.result}
   const heads=tx.objectStore('heads'),head=migrateHead(await req(heads.get('active')) as Head),result=resolveAction(head,request)
