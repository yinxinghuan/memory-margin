...
export type SpaceOptions={world:World;host:HTMLElement;scene:string;position:Point;speed:number;stride:number;poses:string[];sheet:any;spritesheets?:any[];mapEvents?:(scene:string)=>any[];prepareScene?:(scene:string)=>Promise<void>;onFrame?:(client:RpgClientEngine,scene:string,position:Point)=>void;onPosition:(p:Point)=>void;onTravel?:(actualDistance:number)=>void;onDestination:(p:Point|null)=>void;onReady:(space:Space)=>void;onError:(error:unknown)=>void}
...
void wait(scene).then(()=>{project();options.onReady(runtime)}).catch(options.onError)}catch(e){options.onError(e)}}},maps:ids.map(id=>({id,events:options.mapEvents?.(id)??[]}))}])]})
 ...
 const tick=(time:number)=>{
  ...
  project();if(client&&!changing)options.onFrame?.(client,scene,pos);requestAnimationFrame(tick)
 }
