import {discoveryArt} from './exploration'
import {useMapGesture} from './use-map-gesture'
import {useMemo,useState} from 'react'
import type {Scene,EntityId} from './world'
import {sceneLabels,world,entities,portals} from './world'
...
function BackpackThumb({kind,id}:{kind:'person'|'object';id:string}){
 return <i className="mm-backpack-thumb" data-kind={kind} style={{backgroundImage:`url(${discoveryArt[id]??atlas(kind==='person'?`rebuild-20260924/${id}-portrait.png`:'objects.png')})`,backgroundPosition:kind==='person'||discoveryArt[id]?'center':recordCell[id]??'0% 0%',backgroundSize:discoveryArt[id]?'contain':kind==='person'?'cover':undefined,backgroundRepeat:'no-repeat'}} aria-hidden="true"/>
}

const positions:Record<Scene,[number,number]>={home:[17,81],hall:[50,81],service:[83,81],commons:[50,49],'neighbor-room':[17,49],repair:[50,17],archive:[83,49]}
const links:[Scene,Scene][]=[['home','hall'],['hall','service'],['hall','commons'],['commons','neighbor-room'],['commons','repair'],['commons','archive']]
export function mapRoute(scene:Scene,destination:Scene,visited:ReadonlySet<string>){
 const queue:Scene[][]=[[scene]],seen=new Set<Scene>([scene]);if(!visited.has(scene)||!visited.has(destination))return []
 while(queue.length){const path=queue.shift()!,last=path.at(-1)!;if(last===destination)return path
 for(const p of Object.values(portals).filter(p=>p.from===last)){if(visited.has(p.scene)&&!seen.has(p.scene)){seen.add(p.scene);queue.push([...path,p.scene])}}}return []
}
...
export function MemoryMap({save,scene,locale,onTravel,busy=false}:{onTravel:(scene:Scene)=>void;busy?:boolean;save:StorySave;scene:Scene;locale:Locale}){
 ...
 const suggested=(Object.keys(sceneLabels) as Scene[]).find(id=>objective(save).startsWith(sceneLabels[id][locale==='zh'?0:1]))
 const [destination,setDestination]=useState<Scene>(suggested??scene)
 const route=useMemo(()=>{
  if(destination===scene)return [scene]
  return mapRoute(scene,destination,new Set(Object.keys(positions)))
 },[destination,scene,save.map])
 const reachable=mapRoute(scene,destination,visited).at(-1)===destination
 const first=route[1]
 const firstDoor=first?Object.values(entities).find(e=>e.scene===scene&&e.kind==='door'&&e.actions.some(a=>portals[a]?.scene===first)):undefined
 const direction=firstDoor?copy(`从当前房间的${({N:'北',S:'南',E:'东',W:'西'} as const)[firstDoor.side!]}侧门前往${sceneLabels[first!][0]}。`,`Take the ${{N:'north',S:'south',E:'east',W:'west'}[firstDoor.side!]} door to ${sceneLabels[first!][1]}.`):copy('点选地点查看路线；首次亲自到访后可快捷前往。','Select a place for directions. Visit once to unlock quick travel.')
