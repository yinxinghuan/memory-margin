...
function topicCatalog(id:EntityId,save:StorySave):ConversationTopic[]{
 ...
 if(id==='neighbor')return [
  ...(f.keepsakeRead?[{id:'keepsake',label:tx('我听了你留给来客的录音','I heard your visitor recording'),reply:tx('“所以我不会替你选。别人说腾出一个位置很划算，可那一格里装的不是数字。你可以去生活区北边的修复铺先查清楚自己的声音，再决定。”','“Then you know why I will not choose for you. They call an empty slot a bargain, but a slot is not just a number. Check your voice at the repair shop north of the commons before deciding.”')}]:[]),
 ...
 if(id==='caretaker')return [
  ...(f.voiceDiagnosed?[{id:'diagnosis',label:tx('修复铺说声音没有损坏','The repair shop says the voice is intact'),reply:tx('“那就别付修理费。去生活区东边的档案区，查是谁把读取权限移走。检测告诉你没坏，记录才能告诉你是谁动了它。”','“Then do not pay for repairs. Go to the archive east of the commons and see who moved permission. A test proves it is intact. A record tells you who touched it.”')}]:[]),
