...
document.documentElement.dataset.release='memory-skill-validation-20260924-r1'
...
function App(){
 ...
 const apply=async(request:Action)=>{
  ...
  busyRef.current=true;setBusyKind(entities[request.entity]?.kind==='door'?'transition':'action');setBusy(true);space.current.pause(true);setError('')
 ...
 const talk=async(id:EntityId,label:string,authoredReply?:string)=>{
  ...
  try{
   ...
   const next=await store.current.appendConversationExchange(h.version,id,label,text,exchangeId)
 ...
 const conversationHistory=selected&&head?characterConversation(head.conversationHistory,selected.id,8):[]
 ...
 return <main className="mm-app" data-release="memory-skill-validation-20260924-r1" data-spatial-ui-theme="05-terminal"><header className="mm-header"><div><small>{sceneLabels[scene][locale==='zh'?0:1]}</small><h1>{copy('记忆余量','Memory Margin')}</h1></div><nav><button aria-label={copy('地图','Map')} disabled={openingOpen} onClick={()=>setDialog('map')}><Icon name="map"/><span>{copy('地图','Map')}</span></button><button aria-label={copy('背包','Backpack')} disabled={openingOpen} onClick={()=>{setBackpackTab('goal');setDialog('backpack')}}><Icon name="bag"/><span>{copy('背包','Bag')}</span></button><button aria-label={copy('菜单','Menu')} disabled={openingOpen} onClick={()=>setDialog('menu')}><Icon name="menu"/><span>{copy('菜单','Menu')}</span></button><button className="mm-sound" aria-label={muted?copy('开启声音','Turn sound on'):copy('静音','Mute sound')} aria-pressed={!muted} onClick={toggleSound}><Icon name={muted?'mute':'sound'}/></button></nav></header>
  ...
  {dialog&&<div className={selected?'mm-interaction-layer':'mm-scrim'}><section className="mm-dialog" data-kind={selected?.kind} role="dialog" aria-modal={selected?undefined:true} aria-label={selected?entityLabel(selected):panelTitle}><header>{selected?.kind==='person'&&<button className="mm-portrait mm-dialog-portrait" data-character={selected.id} style={{backgroundImage:`url('./art/rebuild-20260924/${selected.id}-portrait.png')`,backgroundSize:'cover',backgroundPosition:'center'}} aria-label={copy('放大人物头像','Enlarge character portrait')} onClick={()=>setZoom('portrait')}><Icon name="expand"/></button>}<div><small>{selected?sceneLabels[scene][locale==='zh'?0:1]:copy('旅程工具','JOURNEY TOOLS')}</small><h2>{selected?entityLabel(selected):panelTitle}</h2></div><button aria-label={copy('回到地图','Back to map')} disabled={busy} onClick={()=>{setDialog(null);setZoom(null);setConfirmation(null);setMessage('');resetConversation();space.current?.pause(false)}}><Icon name="close"/></button></header><div className="mm-dialog-body">
   ...
   {selected&&<>
    ...
    {!(selected.kind==='person'&&conversationReply)&&<p className="mm-message" aria-live="polite">{message||(selected.id==='preview-screen'?(facts.previewStopped?copy('预览已关闭。23:17 的匿名记录还在。','Preview stopped. The anonymous 23:17 log remains.'):facts.previewTraced?copy('预览已关闭。收听账户 C-09 已记下。','Preview stopped. Listener account C-09 is recorded.'):facts.previewLogRead?copy('记录显示：授权空白时，私人声音已外放六秒。','The log shows six seconds played without authorization.'):copy('屏幕还亮着。','The screen is still on.')):revisitText)}</p>}
    {selected.kind==='person'&&conversationPhase==='choices'&&available(selected.id).length>0&&<div className="mm-actions">{available(selected.id).map(a=><button key={a} disabled={busy||Boolean(error)} onClick={()=>act(selected.id,a)}>{actionCopy[a][locale==='zh'?0:1]}<Icon name="arrow"/></button>)}</div>}
    {selected.kind==='person'&&topics.length>0&&<section className="mm-conversation" aria-label={copy('交谈','Conversation')} data-phase={conversationPhase} data-composing={composerOpen}>
     ...
     {conversationPhase==='choices'&&<div className="mm-conversation-options">
      <span className="mm-response-label">{conversationReply?copy('你的回应','YOUR RESPONSE'):copy('选择一句开场','CHOOSE A LINE')}</span>
      <div className="mm-conversation-topics">{talkTopics.slice(0,2).map(topic=><button key={topic.id} onClick={()=>talk(selected.id,topic.label,topic.reply)}>{topic.label}</button>)}</div>
      {talkTopics.length>2&&<><button className="mm-conversation-disclosure" aria-expanded={conversationGroup==='more'} onClick={()=>setConversationGroup(group=>group==='more'?null:'more')}><span>{copy('其他话题','Other topics')}</span><Icon name="arrow"/></button>{conversationGroup==='more'&&<div className="mm-conversation-topics">{talkTopics.slice(2).map(topic=><button key={topic.id} onClick={()=>talk(selected.id,topic.label,topic.reply)}>{topic.label}</button>)}</div>}</>}
      {shareTopics.length>0&&<><button className="mm-conversation-disclosure" aria-expanded={conversationGroup==='share'} onClick={()=>setConversationGroup(group=>group==='share'?null:'share')}><span>{copy('分享发现','Share a discovery')}</span><Icon name="arrow"/></button>{conversationGroup==='share'&&<div className="mm-conversation-topics">{shareTopics.map(topic=><button key={topic.id} onClick={()=>talk(selected.id,topic.label,topic.reply)}>{topic.label}</button>)}</div>}</>}
     ...
     {visibleConversationHistory.length>0&&<details className="mm-conversation-history"><summary>{copy('最近交谈','Recent conversation')}<span>{visibleConversationHistory.length}</span></summary><div>{visibleConversationHistory.map(turn=><p key={turn.id} data-speaker={turn.speaker}><strong>{turn.speaker==='player'?copy('你','You'):conversationName(selected.id,head!.save)}</strong>{turn.text}</p>)}</div></details>}
    </section>}
    ...
    {selected.kind!=='person'&&<div className="mm-actions">{available(selected.id).map(a=><button key={a} disabled={busy||Boolean(error)} onClick={()=>act(selected.id,a)}>{actionCopy[a][locale==='zh'?0:1]}<Icon name="arrow"/></button>)}</div>}
